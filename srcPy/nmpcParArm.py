...
class armNMPC:
    def __init__(self, armParameters, method, Ts, ocpParameters):
        ...
        self.hblock = self.inverseModel(self.cmodel, self.cdata)
    ...
    def inverseModel(self, cmodel, cdata):
        ...
        x_next = ca.SX.sym("x_next", x.shape)

        # RNEA Function
        InvDyn = cpin.rnea(cmodel, cdata, q, v, a)
        ...
        # hk_rnea = ca.Function('rnea', [x, a], [tau], ['x', 'a'], ['tau'], {'custom_jacobian': rneaJac, 'jac_penalty': 0}).expand()
        # eps_dyn = f(xk) - xk+1
        # eps_rnea = InvDyn - u
